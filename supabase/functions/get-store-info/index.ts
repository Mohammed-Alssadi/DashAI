...
serve(async (req: Request) => {
  ...
  try {
    ...
    } else if (isExpired && refresh_token && storeData.platform === "zid") {
      // تجديد التوكن من زد
      // POST https://oauth.zid.sa/oauth/token مع grant_type=refresh_token
      try {
        console.log("تجديد توكن زد...")
        const refreshResponse = await fetch("https://oauth.zid.sa/oauth/token", {
          method: "POST",
        ...
        if (refreshResponse.ok) {
          const refreshData = await refreshResponse.json()
          console.log("تم تجديد توكن زد بنجاح")

          // حسب التوثيق: authorization → Authorization header, access_token → X-Manager-Token
          access_token = refreshData?.authorization ?? access_token
          manager_token = refreshData?.access_token ?? manager_token
          refresh_token = refreshData?.refresh_token ?? refresh_token

          // حفظ التوكنات المحدّثة
          await supabase
            .from("store_tokens")
            .update({
          ...
          console.warn("فشل تجديد توكن زد:", await refreshResponse.text())
        }
      } catch (refreshErr) {
        console.warn("فشل تجديد التوكن من زد:", refreshErr)
      }
    }
    ...
    if (storeData.platform === "salla") {
      ...
      // بناء هيدرات API زد حسب التوثيق الرسمي:
      // access_token في DB = authorization token → Authorization header
      // manager_token في DB = access_token من زد → X-Manager-Token header
      const zidHeaders: Record<string, string> = {
        "Accept": "application/json",
      }
      if (access_token) {
        zidHeaders["Authorization"] = access_token.startsWith("Bearer ")
          ? access_token
          : `Bearer ${access_token}`
      }
      if (manager_token) {
        zidHeaders["X-Manager-Token"] = manager_token
      }
      ...
      if (!storeRes.ok) {
        ...
        console.error("فشل جلب بيانات المتجر الأساسية من زد:", storeRes.status, errorText)
