...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (error || !code || !stateParam) {
      console.error("خطأ في الـ OAuth من سلة أو معاملات مفقودة:", error)
      return Response.redirect(`${frontendUrl}/connect?error=salla_denied`, 302)
    }
    ...
    const { data: storeRecord, error: storeError } = await supabase
      ...
      .upsert(
        {
          user_id: userId, // ربط المتجر بالمستخدم
          platform: "salla",
